<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="课题文件" prop="ketiwenjian">
          <file-upload
          tip="点击上传课题文件"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.ketiwenjian" label="课题文件" prop="ketiwenjian">
            <el-button type="text" size="small" @click="download(ruleForm.ketiwenjian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="课题简介" prop="ketijianjie">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.ketijianjie" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.ketijianjie" label="课题简介" prop="ketijianjie">
                    <span v-html="ruleForm.ketijianjie"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.ketijianjie = this.ruleForm.ketijianjie.replace(reg,'../../../ssmwx41b/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `ketiguanli/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.ketiguanliCrossAddOrUpdateFlag = false;
                  this.parent.search();
